...
export function WorkbookBrowser({
  ...
  const [publisherId, setPublisherId] = useState<string>(initialPublisherId ?? "");
  const [difficultyLevel, setDifficultyLevel] = useState<string>(
    initialLevel != null ? String(initialLevel) : ""
  );
  ...
  const workbooksById = useMemo(
    () => new Map(allWorkbooks.map((w) => [w.id, w])),
    [allWorkbooks]
  );
  ...
  const compareWorkbooks = useMemo(
    ...
    [compareIds, workbooksById]
  );

  const workbooks = useMemo(() => {
    return filterWorkbooks(allWorkbooks, publishers, {
  ...
  }, [allWorkbooks, publishers, search, subjectId, publisherId, difficultyLevel, bookType, sort]);
