    ...
    allSteps.forEach((step, rowIndex) => {
      ...
      nodes.push({
        ...
        data: {
          ...
          publisher: getPublisherName(step.workbook.publisherId),
...
function RoadmapFlowChart({
  ...
  const { nodes: initialNodes, edges: initialEdges, chartHeight } = useMemo(() => {
    ...
    return buildFlatFlow(steps, isLoggedIn, getWorkbookStatus, hoursPerDay, getPublisherName);
  }, [steps, roadmapId, isLoggedIn, getWorkbookStatus, hoursPerDay, getPublisherName]);
...
function RoadmapSection({
  ...
  getSteps: (roadmapId: string) => EnrichedStep[];
  getPublisherName: (id: string) => string;
...
function RoadmapViewInner({
  ...
  const getPublisherName = useCallback(
    (id: string) => publishers.find((p) => p.id === id)?.name ?? "",
    [publishers]
  );

  const getSteps = useCallback(
    (roadmapId: string): EnrichedStep[] =>
      roadmapSteps
        .filter((s) => s.roadmapId === roadmapId)
        .sort((a, b) => a.stepOrder - b.stepOrder)
        .map((s) => {
          const workbook = workbooksById.get(s.workbookId);
          return workbook ? { ...s, workbook } : null;
        })
        .filter((s): s is EnrichedStep => s !== null),
    [roadmapSteps, workbooksById]
  );

  const sortedRoadmaps = useMemo(
    () => [...roadmaps].sort((a, b) => a.displayOrder - b.displayOrder),
    [roadmaps]
  );
  const gradeRoadmaps = useMemo(
    () => sortedRoadmaps.filter((r) => r.type === "grade"),
    [sortedRoadmaps]
  );
  const publisherRoadmaps = useMemo(
    () => sortedRoadmaps.filter((r) => r.type === "publisher"),
    [sortedRoadmaps]
  );
    ...
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      {section === "publisher" && (
        <RoadmapSection
          ...
          onHoursChange={setHoursPerDay}
          renderDescription={(rm) => {
            const publisher = rm.publisherId
              ? publishers.find((p) => p.id === rm.publisherId)
              : null;
            return (
