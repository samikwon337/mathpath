...
export function DashboardWorkbooksView({
  workbooks,
  publishers,
}: {
  workbooks: Workbook[];
  ...
  const publishersById = useMemo(
    () => new Map(publishers.map((p) => [p.id, p])),
    [publishers]
  );
    ...
    const items = userWorkbooks
      .map((uw) => {
        ...
        return {
          ...
          workbook,
          publisher: workbook
            ? publishersById.get(workbook.publisherId)
            : undefined,
        };
      })
  ...
  }, [userWorkbooks, tab, workbooksById, publishersById]);
