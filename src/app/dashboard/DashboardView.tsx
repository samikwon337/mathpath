...
export function DashboardView({ workbooks }: { workbooks: Workbook[] }) {
  ...
  const stats = useMemo(() => {
    ...
    for (const uw of userWorkbooks) {
      const wb = workbooksById.get(uw.workbookId);
      if (!wb) continue;
    ...
    const recentActivity = [...userWorkbooks]
      ...
      .map((uw) => ({
        ...uw,
        workbook: workbooksById.get(uw.workbookId),
      }));
